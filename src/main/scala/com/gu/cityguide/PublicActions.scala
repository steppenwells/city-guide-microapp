...
class PublicActions extends ScalatraFilter with ScalateSupport {
  ...
  get("/images/:city/:cat") {
    ...
    layoutTemplate("templates/catImages.ssp", "data" -> CategoryWithItems(city.get, category.get, randomItems))
  }
...
case class CategoryWithItems(city: City, category: Category, items: List[Item])
